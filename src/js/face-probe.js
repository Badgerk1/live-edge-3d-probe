...
async function runFaceProbe(axis, _calledFromCombined){
  ...
  try{
    ...
    // Average all sample topZ values into a single reference height so every
    // sample probes at the same Z depth — eliminates horizontal/vertical
    // banding caused by per-sample Z variation.
    var _avgTopZSum = 0;
    for(var _ati = 0; _ati < faceSamples.length; _ati++) _avgTopZSum += Number(faceSamples[_ati].topZ);
    var avgTopZ = faceSamples.length ? (_avgTopZSum / faceSamples.length) : 0;
    logLine('face', 'Face probe: averaged topZ reference = ' + avgTopZ.toFixed(3) + ' coords (from ' + faceSamples.length + ' samples, range ' + Math.min.apply(null, faceSamples.map(function(s){ return Number(s.topZ); })).toFixed(3) + ' to ' + Math.max.apply(null, faceSamples.map(function(s){ return Number(s.topZ); })).toFixed(3) + ')');
    ...
    if(s.enableLayeredFace){
      ...
      for(var layerIdx = 0; layerIdx < totalLayers; layerIdx++){
        if(layerIdx === totalLayers - 1){
          ...
          // Use averaged topZ so retract heights are consistent across all samples
          var nextDeepestZ = avgTopZ - maxDepth;
          var nextSpacing = (avgTopZ - 0.05 - nextDeepestZ) / (totalLayers - 1);
