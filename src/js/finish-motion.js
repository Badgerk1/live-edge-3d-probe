...
function faceApplyCompensationCore(gcodeText, contactPoints, referenceContact, overrideAxis, uniformOffset) {
  ...
  // Groups are separated by G0 rapids, G1 moves with Z >= 0 (V-carve retracts),
  // or non-move lines.  Every G1 cutting move (Z < 0) in a group receives the
  // same Y (or X) offset derived from the group centroid, preserving letter geometry.
  ...
  if (uniformOffset) {
    ...
    for (var pi = 0; pi < lines.length; pi++) {
      ...
      // Strip optional N-number prefix (e.g. "N290 G1 X..." → "G1 X...") before
      // G-code command detection so the anchored regex ^G0/^G1 matches correctly.
      var plCore = pl.replace(/^N\d+\s*/i, '');
      var plIsG0 = /^G0\b|^G00\b/i.test(plCore);
      var plIsG1 = /^G1\b/i.test(plCore);
      ...
      if (plIsG0) {
        ...
        if (pz >= 0) {
          // Z >= 0 = retract or reposition (V-carve uses Z=0 between letter strokes)
          // End the current cutting group but don't add this line to any group.
          finalizeGrp();
        } else {
          // Z < 0 = cutting move — add to current group
          grpLines.push(pi);
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    // Strip optional N-number prefix for G-code command detection.
    var lineCore = line.replace(/^N\d+\s*/i, '');
    var isMove = /^G[01]\b/i.test(lineCore) || /\b[XYZ][-\d.]/i.test(line);
    ...
    var isG1Linear = /^G1\b/i.test(lineCore);
