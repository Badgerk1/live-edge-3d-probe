...
async function runOutlineSurfaceGridProbe() {
  ...
  try {
    ...
    var gridEdgeMargin = (function() { var el = document.getElementById('outlineGridEdgeMargin'); return el ? Math.max(0, Number(el.value) || 2.0) : 2.0; })();
    ...
    for (var row = 0; row < gridCfg.rowCount; row++) {
      ...
      // Apply edge margin to Y bounds so rows stay within [minY+edgeMargin, maxY-edgeMargin].
      // If the margin collapses the Y span, fall back to a single row at the overall center.
      var rowYMin2 = gridCfg.minY + gridEdgeMargin;
      var rowYMax2 = gridCfg.maxY - gridEdgeMargin;
      var rowY;
      if (rowYMax2 <= rowYMin2) {
        rowY = (gridCfg.minY + gridCfg.maxY) / 2;
      } else {
        var yEffLen = rowYMax2 - rowYMin2;
        rowY = rowYMin2 + (row + 0.5) * (yEffLen / gridCfg.rowCount);
      }
      // B) Row start log
      ...
      // Apply edge margin to the X span for centered sampling within [x0, x1].
      // If the margin collapses the span (x1 <= x0), fall back to a single point at center.
      var rowEdgeX0 = rowXLeft + gridEdgeMargin;
      var rowEdgeX1 = rowXRight - gridEdgeMargin;
      var rowEdgeMarginDegenerate = (!rowIsDegenerate && rowEdgeX1 <= rowEdgeX0);
      var rowCellWidth;
      if (rowIsDegenerate || rowEdgeMarginDegenerate) {
        rowCellWidth = rowColSpan;
      } else {
        var rowEffSpan = rowEdgeX1 - rowEdgeX0;
        rowCellWidth = gridCfg.colCount > 1 ? rowEffSpan / gridCfg.colCount : rowEffSpan;
      }
      if (rowIsDegenerate) {
      ...
      } else if (rowEdgeMarginDegenerate) {
        outlineAppendLog('ROW span: xLeft=' + rowXLeft.toFixed(3) + ', xRight=' + rowXRight.toFixed(3) +
          ', cols=' + gridCfg.colCount + ', edgeMargin=' + gridEdgeMargin.toFixed(3) + 'mm' +
          ' (Span too small after margin; probing center at X=' + ((rowXLeft + rowXRight) / 2).toFixed(3) + ')');
      } else {
        outlineAppendLog('ROW span: xLeft=' + rowXLeft.toFixed(3) + ', xRight=' + rowXRight.toFixed(3) +
      ...
      for (var step = 0; step < gridCfg.colCount; step++) {
        outlineCheckStop();
        var col = step;
        // If span is degenerate (xLeft ≈ xRight) or edge margin collapses the span,
        // only probe the first column; skip the rest to avoid duplicate X positions.
        if ((rowIsDegenerate || rowEdgeMarginDegenerate) && col > 0) {
        ...
        // Column X: centered within [rowEdgeX0, rowEdgeX1] when edge margin is valid.
        // Falls back to span midpoint for degenerate or margin-collapsed spans.
        var colX;
        if (rowIsDegenerate || rowEdgeMarginDegenerate || gridCfg.colCount === 1) {
          colX = (rowXLeft + rowXRight) / 2;
        } else {
          colX = rowEdgeX0 + (col + 0.5) * rowCellWidth;
        }
