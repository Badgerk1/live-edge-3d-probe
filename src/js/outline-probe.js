...
async function runOutlineSurfaceGridProbe() {
  ...
  try {
    ...
    if (gridSource === 'detected' && hasOutlineData) {
      ...
      outlineAppendLog('Grid source: DETECTED OUTLINE (margin=' + gridMargin + 'mm) \u2014 ' +
        allX.length + ' edge points from ' + outlineRowResults.length + ' rows / ' + outlineColResults.length + ' cols');
      outlineAppendLog('Grid bounds from detected outline: X' + gridMinX.toFixed(3) + '\u2192' + gridMaxX.toFixed(3) + '  Y' + gridMinY.toFixed(3) + '\u2192' + gridMaxY.toFixed(3));
    } else {
    ...
    outlineAppendLog('Bounds source: ' + (gridSourceUsed === 'detected' ? 'DETECTED OUTLINE (auto-inset ' + gridMargin + 'mm)' : 'OUTLINE SEARCH BOUNDS (fallback)'));
    ...
    for (var row = 0; row < gridCfg.rowCount; row++) {
      ...
      for (var step = 0; step < gridCfg.colCount; step++) {
        ...
        // Outline-local plunge probe — uses G38.2 with Outline-derived settings only.
        // Never reads Surface Probe tab DOM fields (sm-clearanceZ, travelRecoveryLiftFeedRate, etc.)
        var pos = await _outlinePlungeProbe(maxPlunge, probeFeed, clearanceZ);
