...
function exportSurfaceDXF(){
  if(!smMeshData || !smGridConfig){ setFooterStatus('No surface mesh data to export.', 'warn'); return; }
  ...
  var a = document.createElement('a');
  a.href = url;
  a.download = 'surface_mesh_' + tsForFilename() + '.dxf';
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
...
function exportSurfaceOBJ(){
  ...
  var resEl = document.getElementById('surfOBJSubdivision');
  var resolution = resEl ? parseFloat(resEl.value) : 2;
  if(isNaN(resolution) || resolution <= 0) resolution = 2;

  var obj = '# 3D Live Edge Mesh - Surface Profile\n';
  obj += '# Plugin Version: ' + SM_VERSION + '\n';
  obj += '# Generated ' + new Date().toISOString() + '\n';
  obj += 'o SurfaceMesh\n';
  obj += 'g SurfaceMesh\n';

  // Build vertex and index arrays directly from the surface probe grid
  var indexGrid = [];
  var vIdx = 1;
  var totalVerts = 0;
  var subRows = [];
  ...
  for(var ri = 0; ri < cfg.rowCount; ri++){
    ...
    for(var ci = 0; ci < cfg.colCount; ci++){
      var z = smMeshData[ri][ci];
      if(z != null){
        subRow.push({ x: cfg.minX + ci * cfg.colSpacing, y: cfg.minY + ri * cfg.rowSpacing, z: z });
      } else {
        subRow.push(null);
      }
    }
  ...
  // Emit vertices and build index grid
  for(var ri = 0; ri < subRows.length; ri++){
    indexGrid.push([]);
    for(var ci = 0; ci < subRows[ri].length; ci++){
      var p = subRows[ri][ci];
      if(p){
        obj += 'v ' + p.x.toFixed(3) + ' ' + p.y.toFixed(3) + ' ' + p.z.toFixed(3) + '\n';
        indexGrid[ri].push(vIdx++);
  ...
  // Triangulate adjacent quads
  var normals = [];
  var faces = [];
  for(var ri = 0; ri < subRows.length - 1; ri++){
    for(var ci = 0; ci < subRows[ri].length - 1; ci++){
      var va = indexGrid[ri][ci], vb = indexGrid[ri][ci + 1];
      var vc = indexGrid[ri + 1][ci + 1], vd = indexGrid[ri + 1][ci];
      if(va && vb && vc){
        var pa = subRows[ri][ci], pb = subRows[ri][ci + 1], pc = subRows[ri + 1][ci + 1];
        var e1x = pb.x - pa.x, e1y = pb.y - pa.y, e1z = pb.z - pa.z;
        var e2x = pc.x - pa.x, e2y = pc.y - pa.y, e2z = pc.z - pa.z;
        var nx = e1y * e2z - e1z * e2y, ny = e1z * e2x - e1x * e2z, nz = e1x * e2y - e1y * e2x;
        var len = Math.sqrt(nx * nx + ny * ny + nz * nz);
      ...
      if(va && vc && vd){
        var pa = subRows[ri][ci], pc = subRows[ri + 1][ci + 1], pd = subRows[ri + 1][ci];
        var e1x = pc.x - pa.x, e1y = pc.y - pa.y, e1z = pc.z - pa.z;
        var e2x = pd.x - pa.x, e2y = pd.y - pa.y, e2z = pd.z - pa.z;
        var nx = e1y * e2z - e1z * e2y, ny = e1z * e2x - e1x * e2z, nz = e1x * e2y - e1y * e2x;
        var len = Math.sqrt(nx * nx + ny * ny + nz * nz);
  ...
  var anchor = document.createElement('a');
  anchor.href = url;
  anchor.download = 'surface_mesh_' + tsForFilename() + '.obj';
  document.body.appendChild(anchor);
  anchor.click();
  document.body.removeChild(anchor);
  URL.revokeObjectURL(url);
