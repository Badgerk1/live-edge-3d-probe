...
async function _getState(){
  ...
  var ms = _machineStateFrom(state);
  pluginDebug('_getState raw: status=' + (ms.status || 'n/a') + ' WPos=' + String(ms.WPos || 'n/a') + ' MPos=' + String(ms.MPos || 'n/a'));
  return state;
}
...
function _machineStateFrom(state){
  ...
  var candidates = [
    state.machineState,
    state.cnc && state.cnc.machineState,
    state.cnc && state.cnc.state,
    state.cnc && state.cnc.status,
    state.controller && state.controller.machineState,
    state.controller && state.controller.state,
    state.controller && state.controller.status,
    state.grbl && state.grbl.machineState,
    state.grbl
  ];
  var best = {};
  var bestScore = -1;
  for(var i = 0; i < candidates.length; i++){
    ...
    var normalized = _normalizeMachineState(c);
    var score = _machineStateScore(normalized);
    if(score > bestScore){
      best = normalized;
      bestScore = score;
    }
...
function _machineStateScore(ms){
  if(!ms || typeof ms !== 'object') return -1;
  var score = 0;
  if(ms.status != null) score += 2;
  if(ms.Pn != null) score += 1;
  if(ms.WPos) score += 3;
  if(ms.MPos) score += 3;
  if(ms.WCO) score += 1;
  var w = _parsePos(ms.WPos);
  var m = _parsePos(ms.MPos);
  if(w && (Math.abs(w.x) > 0.0005 || Math.abs(w.y) > 0.0005 || Math.abs(w.z) > 0.0005)) score += 4;
  if(m && (Math.abs(m.x) > 0.0005 || Math.abs(m.y) > 0.0005 || Math.abs(m.z) > 0.0005)) score += 4;
  return score;
}

function _pickMsField(ms, keys){
...
function _normalizeMachineState(ms){
  if(!ms || typeof ms !== 'object') return {};
  var out = Object.assign({}, ms);
  var status = _pickMsField(ms, ['status', 'Status', 'state', 'machineStatus']);
  var probePins = _pickMsField(ms, ['Pn', 'pn', 'pins']);
  var wPos = _normalizeMsPos(_pickMsField(ms, ['WPos', 'wPos', 'wpos', 'workPos', 'workPosition']));
  var mPos = _normalizeMsPos(_pickMsField(ms, ['MPos', 'mPos', 'mpos', 'machinePos', 'machinePosition']));
  var wco  = _normalizeMsPos(_pickMsField(ms, ['WCO', 'wco', 'workOffset', 'workCoordinateOffset']));
  if(status != null) out.status = String(status);
  if(probePins != null) out.Pn = String(probePins);
...
function _machineAlarmReason(ms, state){
  var parts = [];
  function addCandidate(v){
    ...
    if(lower.indexOf('alarm') >= 0 || lower.indexOf('error') >= 0) parts.push(text);
  }
  addCandidate(ms && ms.status);
  addCandidate(ms && ms.alarm);
  addCandidate(ms && ms.alarmMessage);
  addCandidate(ms && ms.lastAlarm);
  addCandidate(ms && ms.error);
  addCandidate(ms && ms.lastError);
  addCandidate(state && state.alarm);
  addCandidate(state && state.alarmMessage);
  addCandidate(state && state.lastAlarm);
  addCandidate(state && state.error);
  addCandidate(state && state.lastError);
  if(parts.length === 0) return '';
  return parts.filter(function(v, i, arr){ return arr.indexOf(v) === i; }).join(' | ');
}
...
async function getMachineSnapshot(){
  ...
  var state = await _getState();
  var ms = _machineStateFrom(state);
  var alarmReason = _machineAlarmReason(ms, state);
  ...
  pluginDebug('getMachineSnapshot EXIT: status=' + snap.status + ' pos=X' + snap.x + ' Y' + snap.y + ' Z' + snap.z + ' homed=' + snap.homed + ' probe=' + snap.probeTriggered + ' feedOvr=' + snap.feedOverridePct + '% rapidOvr=' + snap.rapidOverridePct + '% spindleOvr=' + snap.spindleOverridePct + '%' + (snap.alarmReason ? ' alarmReason="' + snap.alarmReason + '"' : ''));
  return snap;
}
...
function _parsePos(str){
  ...
  if(Array.isArray(str)){
    ...
    var ax = Number(str[0]), ay = Number(str[1]), az = Number(str[2]);
    return isFinite(ax) && isFinite(ay) && isFinite(az) ? {x:ax, y:ay, z:az} : null;
  }
  if(typeof str === 'object'){
    var ox = Number(str.x), oy = Number(str.y), oz = Number(str.z);
    return isFinite(ox) && isFinite(oy) && isFinite(oz) ? {x:ox, y:oy, z:oz} : null;
  }
  if(typeof str !== 'string') return null;
  var p = str.split(',').map(function(v){ return parseFloat(String(v).trim()); });
...
async function getWorkPosition(){
  ...
  var alarmReason = _machineAlarmReason(ms, state);
  var w = _parsePos(ms.WPos);
  if(w) return {x:w.x, y:w.y, z:w.z, status: status, probeTriggered: probeTriggered, alarmReason: alarmReason};
  var m = _parsePos(ms.MPos), wco = _parsePos(ms.WCO);
  if(m && wco) return {x:m.x-wco.x, y:m.y-wco.y, z:m.z-wco.z, status: status, probeTriggered: probeTriggered, alarmReason: alarmReason};
  if(m) return {x:m.x, y:m.y, z:m.z, status: status, probeTriggered: probeTriggered, alarmReason: alarmReason};
...
async function waitForIdle(){
  ...
  for(var i = 0; i < maxPolls; i++){
    ...
    if(status === 'alarm'){
      var alarmReason = _machineAlarmReason(ms, state);
      pluginDebug('waitForIdle ERROR: machine in alarm state' + (alarmReason ? ' reason="' + alarmReason + '"' : ''));
      throw new Error('Machine in alarm state' + (alarmReason ? ': ' + alarmReason : ''));
    }
  }
